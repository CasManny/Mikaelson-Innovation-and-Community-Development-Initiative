        ...
        <div className="flex flex-col gap-8">
            {posts.map((post) => (
                <div key={post.id} className=" bg-white rounded-2xl p-6 border border-[#f3f4f6] shadow-md transition-all">
                    ...
                    <div className="post-tags flex flex-wrap gap-4">
                        {post.tags.map((tag, index) => (
                            <div key={index} className="bg-gray-200 text-black/40 p-1 hover:bg-teal-300 hover:text-white cursor-pointer transition-all rounded-lg">
                                #{tag}
                            </div>
                        ))}
                    </div>
